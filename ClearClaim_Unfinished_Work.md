...
- **Backend PDF Export Robustness:** Refactored the export endpoint to use `Response` with direct binary delivery of PDF bytes generated via `reportlab`. Added a try/except block with detailed traceback logging to stderr for easier production debugging on Render. Confirmed `reportlab` dependency in `requirements.txt`.
