...
- **PDF Export Fix (Binary Integrity) & CORS Hardening:** Resolved PDF corruption issues by refactoring the frontend `ExportControls.tsx` to use native `fetch` with `response.blob()`. Refactored backend `routes/export.py` to use `Response` with direct binary delivery of PDF bytes generated via `reportlab`. Hardened `main.py` CORS configuration to strictly include production origins and explicitly `expose_headers` required for cross-origin downloads. Updated `routes/export.py` to send `X-Suggested-Filename`.
...
- **Backend PDF Export Robustness & Formatting:** Refactored the export endpoint to use `Response` with direct binary delivery of PDF bytes generated via `reportlab`. Implemented multi-line text wrapping for validation messages using `reportlab.platypus.Paragraph` and established explicit `colWidths` (`[120, 60, 80, 210]`) to prevent truncation and text overlap in the 'Error Code' column. Added a try/except block with detailed traceback logging to stderr for easier production debugging on Render. Confirmed `reportlab` dependency in `requirements.txt` (manually updated).
...
- [x] PDF Formatting & Text Wrapping
