...
- **PDF Export Fix:** Updated the backend `/api/export/pdf` endpoint to return the generated PDF bytes with correct headers (`Content-Disposition`, `media_type="application/pdf"`). Refactored the frontend `ExportControls.tsx` to handle the `blob` response, explicitly setting the type and using a programmatic anchor tag flow for secure downloads.
